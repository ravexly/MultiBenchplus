...
from encoders.stacked_image_cnn import StackedImageCNNEncoder
...
def build_encoders() -> list[nn.Module]:
    ...
    return [StackedImageCNNEncoder([32, 64, 128], 128).to(device), StackedImageCNNEncoder([32, 64, 128], 128).to(device), nn.Identity().to(device)]
...
def build_fusion_methods(num_classes: int) -> dict[str, nn.Module]:
    return {
        ...
        "HierarchicalAttentionOneToMulti": HierarchicalAttentionOneToMulti([128, 128, 17]),
        "MultiModalCrossAttentionFusion": MultiModalCrossAttentionFusion([128, 128, 17]),
        "MultiModalCrossAttentionConcatFusion": MultiModalCrossAttentionConcatFusion([128, 128, 17]),
        "EarlyFusionTransformer": EarlyFusionTransformer(273),
        "TMC": TMC(num_classes),
        "LateFusion": MultimodalLateFusionClf(),
        "HierarchicalAttentionMultiToOne": HierarchicalAttentionMultiToOne([128, 128, 17]),
    }
