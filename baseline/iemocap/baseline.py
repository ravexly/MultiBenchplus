...
ROOT, SOURCE_DATASET_DIR, OUTPUT_DIR = setup_baseline_runtime("iemocap", OUTPUT_DIR)
...
from unimodals.common_models import GRUWithLinear, MLP , Identity, Linear 
...
from fusions.common_fusions import (
    ...
    HierarchicalAttentionOneToMulti, LateFusionTransformer,MultiModalCrossAttentionFusion,MultiModalCrossAttentionConcatFusion
)
...
def get_head(num_classes, decision=False):
    if decision:
        return [LazyLinear(num_classes).cuda(), LazyLinear(num_classes).cuda(), LazyLinear(num_classes).cuda()]
    else:
...
def build_encoders():
    ...
    return [Identity().cuda(), Identity().cuda(), Identity().cuda()]
