...
def build_encoders() -> list[nn.Module]:
    return [CompactBranchCNNEncoder(64, 16).to(device), CompactBranchCNNEncoder(64, 16).to(device)]
...
def build_fusion_methods(num_classes: int) -> dict[str, nn.Module]:
    return {
        ...
        "HierarchicalAttentionOneToMulti": HierarchicalAttentionOneToMulti([16, 16]),
        "CrossAttentionFusion": CrossAttentionFusion([16, 16]),
        "CrossAttentionConcatFusion": CrossAttentionConcatFusion([16, 16]),
        "EarlyFusionTransformer": EarlyFusionTransformer(32),
        "TMC": TMC(num_classes),
        "LateFusion": MultimodalLateFusionClf(),
        "HierarchicalAttentionMultiToOne": HierarchicalAttentionMultiToOne([16, 16]),
    }
